...
background = np.array([4, 4, 4])  # label 0
LV = np.array([170, 170, 170]) # label 1
Myo = np.array([113, 113, 113]) # label 2
RV = np.array([56, 56, 56]) # label 3
num_classes = 4
...
def rgb_to_2D_label(_label):
    ...
    label_seg[np.all(_label.transpose([1, 2, 0]) == LV, axis=-1)] = 1
    label_seg[np.all(_label.transpose([1, 2, 0]) == Myo, axis=-1)] = 2
    label_seg[np.all(_label.transpose([1, 2, 0]) == RV, axis=-1)] = 3
    return label_seg
